...
usage() {
  cat <<'EOF'
用法：
  ./scripts/build-desktop.sh [--skip-install] [--help] [-- <tauri build 参数>]

说明：
  - 默认使用上级 TogoSpace 仓库作为 TOGOSPACE_ROOT
  - 自动补齐 apps/desktop/.env
  - 默认执行 npm install，然后调用 tauri build

示例：
  ./scripts/build-desktop.sh
  ./scripts/build-desktop.sh -- --bundles dmg
  ./scripts/build-desktop.sh --skip-install -- --debug
EOF
}
...
if [ ! -f "$TOGOSPACE_ROOT/src/backend_main.py" ]; then
  echo "未找到可打包的 TogoSpace 仓库：$TOGOSPACE_ROOT" >&2
