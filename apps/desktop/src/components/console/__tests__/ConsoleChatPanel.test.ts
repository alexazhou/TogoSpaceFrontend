...
describe('ConsoleChatPanel', () => {
  ...
  it('appends dropped file paths into the composer', async () => {
    const wrapper = mount(ConsoleChatPanel, {
      props: {
        currentRoom: createRoom(),
        agents: [],
    ...
    const composer = wrapper.find('form');
    await composer.trigger('drop', {
      dataTransfer: {
        files: [
          { path: '/Users/demo/Documents/spec.md' },
    ...
    expect((wrapper.find('textarea').element as HTMLTextAreaElement).value).toBe(
      '/Users/demo/Documents/spec.md\n/Users/demo/Documents/notes.txt',
    );
  });

  it('falls back to dropped file names when absolute paths are unavailable', async () => {
    const wrapper = mount(ConsoleChatPanel, {
      props: {
        currentRoom: createRoom(),
        agents: [],
    ...
    await composer.trigger('drop', {
      dataTransfer: {
        files: [
          { name: 'slides.pptx', path: '' },
          { name: 'notes.txt' },
        ],
        items: [
          { kind: 'file', type: 'application/vnd.openxmlformats-officedocument.presentationml.presentation' },
    ...
    expect((wrapper.find('textarea').element as HTMLTextAreaElement).value).toBe('slides.pptx\nnotes.txt');
  });
});
