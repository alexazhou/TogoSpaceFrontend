...
describe('MarkdownContent', () => {
  ...
  it('converts standalone local file paths into folder links', async () => {
    const wrapper = mount(MarkdownContent, {
      props: {
        content: '/Users/demo/My Project/spec.md',
      },
      global: {
        plugins: [i18n],
      },
    ...
    const link = wrapper.find('.markdown-local-path');
    expect(link.exists()).toBe(true);
    expect(link.text()).toBe('/Users/demo/My Project/spec.md');
    expect(link.attributes('href')).toBe('file:///Users/demo/My%20Project');
    expect(link.attributes('title')).toBe('打开所在文件夹');
  });

  it('does not convert code block paths into folder links', async () => {
    const wrapper = mount(MarkdownContent, {
      props: {
        content: '```text\n/Users/demo/My Project/spec.md\n```',
      },
      global: {
        plugins: [i18n],
      },
    ...
    expect(wrapper.find('.markdown-local-path').exists()).toBe(false);
    expect(wrapper.find('pre.hljs code').text()).toContain('/Users/demo/My Project/spec.md');
  });
