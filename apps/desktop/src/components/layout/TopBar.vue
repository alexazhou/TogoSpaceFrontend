...
const props = defineProps<{
  ...
  desktopBackendRunning?: boolean;
  desktopConnected?: boolean;
  desktopStatus?: DesktopStatus | null;
  desktopCommandPending?: 'start' | 'stop' | 'open' | null;
}>();
...
const desktopIndicatorState = computed(() => (props.desktopConnected ? 'connected' : 'disconnected'));
const desktopStatusTitle = computed(() => (
  props.desktopConnected ? t('topbar.desktopConnected') : t('topbar.desktopDisconnected')
));
const desktopBackendLabel = computed(() => (
  props.desktopBackendRunning ? t('topbar.desktopBackendRunning') : t('topbar.desktopBackendStopped')
));
const desktopConnectionLabel = computed(() => (
  props.desktopConnected ? t('connection.connected') : props.statusLabel
));
...
function handleWindowPointerDown(event: PointerEvent): void {
  ...
  if (!target.closest('.desktop-status-menu')) {
  ...
  <header class="topbar" :class="{ 'topbar-console': showConnectionStatus }">
    ...
    <div class="status-group">
      ...
      <div class="topbar-utility-group">
        <div v-if="isTauriDesktop" class="desktop-status-menu">
          <button
            type="button"
            class="nav-icon-button toolbar-icon-button desktop-status-button"
            :class="{ 'is-connected': desktopIndicatorState === 'connected' }"
          ...
          >
            <svg class="desktop-status-button__icon" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M4 6.75A1.75 1.75 0 0 1 5.75 5h12.5A1.75 1.75 0 0 1 20 6.75v8.5A1.75 1.75 0 0 1 18.25 17H5.75A1.75 1.75 0 0 1 4 15.25Z" />
              <path d="M9 19h6" />
              <path d="M12 17v2" />
            </svg>
            <span class="desktop-status-button__dot" :data-state="desktopIndicatorState"></span>
          </button>
          <div
            v-if="desktopMenuOpen"
            class="desktop-status-popover"
            role="dialog"
            aria-live="polite"
          >
            <div class="desktop-status-popover__section">
              <div class="desktop-status-popover__headline">
                <span class="desktop-status-popover__state-dot" :data-state="desktopIndicatorState"></span>
                <div class="desktop-status-popover__headline-copy">
                  <strong>{{ desktopStatusTitle }}</strong>
                  <span>{{ desktopBackendLabel }}</span>
                </div>
              </div>
              <div class="desktop-status-popover__meta-list">
                <span>{{ desktopConnectionLabel }}</span>
                <span v-if="desktopStatus?.backendPid">PID {{ desktopStatus.backendPid }}</span>
                <span v-if="desktopStatus?.appVersion">{{ t('topbar.desktopVersion', { version: desktopStatus.appVersion }) }}</span>
                <code v-if="desktopStatus?.togospaceRoot" class="desktop-status-popover__path">{{ desktopStatus.togospaceRoot }}</code>
              </div>
            </div>
            <div class="desktop-status-popover__actions">
              <button
                type="button"
                class="desktop-status-popover__action desktop-status-popover__action--primary"
                :disabled="desktopCommandPending !== null || desktopBackendRunning"
                @click="emit('startLocalBackend'); closeDesktopMenu()"
              >
                {{ desktopCommandPending === 'start' ? t('topbar.desktopStartPending') : t('topbar.desktopStartBackend') }}
              </button>
              <button
                type="button"
                class="desktop-status-popover__action"
                :disabled="desktopCommandPending !== null || !desktopBackendRunning"
                @click="emit('stopLocalBackend'); closeDesktopMenu()"
              >
                {{ desktopCommandPending === 'stop' ? t('topbar.desktopStopPending') : t('topbar.desktopStopBackend') }}
              </button>
              <button
                type="button"
                class="desktop-status-popover__action"
                :disabled="desktopCommandPending !== null"
                @click="emit('openLocalRoot'); closeDesktopMenu()"
              >
                {{ desktopCommandPending === 'open' ? t('topbar.desktopOpenRootPending') : t('topbar.desktopOpenRoot') }}
              </button>
            </div>
          </div>
        </div>
...
.console-view-switcher {
  ...
  border-radius: 13px;
  background: color-mix(in srgb, var(--surface-panel-muted) 86%, var(--surface-panel) 14%);
  pointer-events: auto;
}
...
.team-switcher-button:hover {
  ...
  background: color-mix(in srgb, var(--interactive-selected) 74%, var(--surface-panel-muted) 26%);
  color: var(--text-primary);
}
...
.nav-icon-button--bare:not(:disabled):hover {
  background: color-mix(in srgb, var(--interactive-selected) 66%, transparent);
  color: var(--text-primary);
}
...
.toolbar-icon-button:not(:disabled):hover {
  border-color: var(--interactive-focus-border);
  background: color-mix(in srgb, var(--interactive-selected) 70%, var(--surface-panel-muted) 30%);
  color: var(--text-primary);
}
...
.desktop-status-button__dot,
.desktop-status-popover__state-dot {
  border-radius: 999px;
...
.desktop-status-button__dot {
  ...
  width: 7px;
  height: 7px;
  border: 1px solid var(--surface-pill);
}

.desktop-status-button__dot[data-state='connected'],
.desktop-status-popover__state-dot[data-state='connected'] {
...
.desktop-status-popover__section {
  display: grid;
  gap: 10px;
}

.desktop-status-popover__headline {
  display: flex;
  align-items: center;
  gap: 10px;
}
...
.desktop-status-popover__headline-copy {
  min-width: 0;
  display: grid;
  gap: 2px;
}

.desktop-status-popover__headline-copy strong {
  color: var(--text-primary);
  font-size: 0.82rem;
}

.desktop-status-popover__headline-copy span,
.desktop-status-popover__meta-list {
  color: var(--text-secondary);
  font-size: 0.72rem;
}
...
.desktop-status-popover__actions {
  display: grid;
  gap: 6px;
}

.desktop-status-popover__action {
  min-height: 30px;
  border: 1px solid var(--border-subtle);
  border-radius: 10px;
  background: color-mix(in srgb, var(--surface-panel-muted) 84%, var(--surface-panel) 16%);
...
.desktop-status-popover__action:hover:not(:disabled) {
  border-color: var(--interactive-focus-border);
  background: color-mix(in srgb, var(--interactive-selected) 70%, var(--surface-panel-muted) 30%);
}

.desktop-status-popover__action--primary {
  border-color: color-mix(in srgb, var(--state-info) 44%, var(--border-subtle) 56%);
  background: color-mix(in srgb, var(--state-info) 12%, var(--surface-panel-muted) 88%);
}

.desktop-status-popover__action:disabled {
  cursor: not-allowed;
  opacity: 0.48;
}
...
.theme-switch {
  ...
  border: 1px solid var(--border-subtle);
  border-radius: 999px;
  background: color-mix(in srgb, var(--surface-glass) 82%, var(--surface-pill) 18%);
  color: var(--text-secondary);
  cursor: pointer;
  outline: none;
  box-shadow: none;
...
:global(html.bp-console-mobile) .topbar.topbar-console .desktop-status-popover,
:global(html.bp-console-short) .topbar.topbar-console .desktop-status-popover {
  right: 0;
}
